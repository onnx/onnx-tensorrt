...
int SkipLayerNormPlugin::enqueue(int batchSize, const void* const* inputs, void** outputs, void*, cudaStream_t stream)
{
    ...
    if (mType == DataType::kFLOAT)
    {
        ...
        //sds: mLd是512，mInputVolume是输入总长度
...
{
    return 2 * sizeof(float) * mLd + sizeof(DataType) + sizeof(mLd) + sizeof(mInputVolume) + getBaseSerializationSize();;
}
