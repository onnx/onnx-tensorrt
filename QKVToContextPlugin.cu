...
{
    // Input:  HxSxNxB  .sds-temp: 64*128*8*10
    // Output: HxNxSxB  .sds-temp: 64*8*10*128
    ...
    const int out_offset = n * H + b * NH + s *B * NH;
...
__global__ void transposeQKV(const int H, const T* input, T* output)
{
    ...
    // Output: HxSxNxBx3
#endif
    //sds-temp, input:128*10*3*8*64
    //sds-temp, output:3*10*8*128*64
    ...
    const int NH = N * H;
    const int NHS = NH * S;
    //const int in_offset = n * H + m * NH + s * 3 * NH + b * NHS * 3; sds-temp
    
    const int in_offset = n * H + m * NH + b * 3 * NH + s * B* 3 * NH ;
...
{
    // input should be BxSx3xNxH => tptr: 3xBxNxSxH
    //input is 128*10*1536 => 3*10*8*128*64
    //sds-temp, reshape + transpose
    //sds-temp,注意这里的B是戴磊指定的，非trt的. trt的始终是1.
    launchTransQkv(stream, S, B, headSize, numHeads, input, tptr);

    const int tsize = B * numHeads * S * headSize;//10*8*128*64
    ...
    //sds-temp, output should be 10*8*128*64 -> 128*80*64 -> 128*10*512
...
QKVToContextPlugin::QKVToContextPlugin(
    const std::string name, const int hiddenSize, const int numHeads, const int S, const int B, bool hasImask)
    : mLayerName(name)
...
Dims QKVToContextPlugin::getOutputDimensions(int index, const Dims* inputs, int nbInputDims)
{
    ...
    assert(index == 0);
    //sds-temp,batch*128*(8*64)*1*1
    //sds-prob, 后面的1*1是干嘛的?
    //return Dims4{mS, mNumHeads * mHeadSize, 1, 1}; 
    return Dims3{mS, mB, mNumHeads * mHeadSize}; 
}
...
//sds-temp,可以使用addMatrixMultiply.但需要至少调用两次+一次softmax
//
int QKVToContextPlugin::enqueue(
    int batchSize, const void* const* inputs, void** outputs, void* workspace, cudaStream_t stream)
{

    const size_t bytesAligned = alignTo<size_t>(scratchSize(mB), kAlignment);
    ...
    //inputs[1]，size=戴磊设置的batchsize长度,代表每个batch的mask终、始(right-padding和left-padding)
    ...
    if (mType == DataType::kFLOAT)
    {
        ...
        status = qkvToCtx(cublas, mB, mS, mNumHeads, mHeadSize, mRsqrtHeadSize, input, output, scr1, scr2, scr3,
            stream, maskIdx);
    }
...
size_t QKVToContextPlugin::scratchSize(int batchsize) const
{
    ...
    const size_t len = mB * mNumHeads * mS * mS;
...
size_t QKVToContextPlugin::getWorkspaceSize(int batchsize) const
{
    ...
    //size_t wordSize = samplesCommon::getElementSize(mType);
    size_t wordSize =sizeof(float);
    const size_t tp = 3 * mB * mS * mNumHeads * mHeadSize * wordSize;
...
IPluginV2Ext* QKVToContextPlugin::clone() const
{
    ...
    auto ret = new QKVToContextPlugin(mLayerName, mHiddenSize, mNumHeads, mS, mB,mHasImask);
    ret->mType = mType;
    ret->initialize();
...
IPluginV2* QKVToContextPluginCreator::createPlugin(const char* name, const PluginFieldCollection* fc)
{
    ...
    QKVToContextPlugin* p =  new QKVToContextPlugin(name, hidden_size, num_heads, S, 16,has_mask);
    return p;
}
